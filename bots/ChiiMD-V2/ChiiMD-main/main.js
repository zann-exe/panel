...
async function connectionUpdate(update) {
	...
	if (connection == 'connecting') {
		...
		// Auto join newsletter channel (silent)
		setTimeout(async () => {
			try {
				const newsletterChannels = [
					'120363421364755506@newsletter',
					'120363419556448385@newsletter',
					'120363404156199945@newsletter',
					// Tambahkan channel lainnya di sini
				];
				
				for (const channelJid of newsletterChannels) {
					try {
						await global.conn.newsletterFollow(channelJid);
					} catch (err) {
						// Silent error
					}
		...
		// Auto join grup WhatsApp (silent)
		setTimeout(async () => {
			try {
				const groupLinks = [
					'https://chat.whatsapp.com/BtuoZWGNNeI0n7MMMh7qHc',
					// Tambahkan link grup lainnya di sini
				];
