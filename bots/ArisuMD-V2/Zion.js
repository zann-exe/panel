...
// Tambahin ini di bagian atas (setelah deklarasi variabel groupMetadata)
// Fix untuk bot admin check yang lebih akurat

// ========== ULTIMATE BOT ADMIN CHECK - HANDLES ALL FORMATS ==========
const UltimateBotAdminCheck = async () => {
    if (!isGroup) return false;
    try {
        const groupMeta = await Zion.groupMetadata(m.chat);
        const participants = groupMeta.participants;
        const adminList = participants.filter(v => v.admin !== null);
        
        // Ambil semua kemungkinan bot number
        let botNumbers = [];
        
        // 1. Dari botNumber
        if (botNumber) {
            botNumbers.push(botNumber);
            // Remove @s.whatsapp.net dan coba format lain
            const cleanBot = botNumber.replace('@s.whatsapp.net', '').replace('@c.us', '');
            botNumbers.push(cleanBot);
            botNumbers.push(cleanBot + '@lid');
            botNumbers.push(cleanBot + '@g.us');
            botNumbers.push(cleanBot + '@c.us');
        }
        
        // 2. Dari Zion.user.id
        if (Zion.user?.id) {
            botNumbers.push(Zion.user.id);
            // Remove :27 part dan domain
            const cleanUser = Zion.user.id.replace(/:.*@.*/, '');
            botNumbers.push(cleanUser);
            botNumbers.push(cleanUser + '@lid');
            botNumbers.push(cleanUser + '@g.us');
            botNumbers.push(cleanUser + '@s.whatsapp.net');
        }
        
        // 3. Extract number only dan coba semua format
        const numberOnly = botNumber?.match(/\d+/)?.[0];
        if (numberOnly) {
            botNumbers.push(numberOnly + '@lid');
            botNumbers.push(numberOnly + '@s.whatsapp.net');
            botNumbers.push(numberOnly + '@c.us');
            botNumbers.push(numberOnly + '@g.us');
        }
        
        // Remove duplicates
        botNumbers = [...new Set(botNumbers.filter(n => n))];
        
        console.log('🔧 Ultimate Bot Admin Check:');
        console.log('- All Bot Numbers to Check:', botNumbers);
        console.log('- Admin List:', adminList.map(a => a.id));
        
        // Check apakah ada yang match dengan admin
        const isBotAdmin = adminList.some(admin => 
            botNumbers.some(botNum => {
                // Exact match
                if (admin.id === botNum) return true;
                
                // Number only match (ignore domain)
                const adminNumber = admin.id.match(/\d+/)?.[0];
                const botNumberOnly = botNum.match(/\d+/)?.[0];
                
                return adminNumber && botNumberOnly && adminNumber === botNumberOnly;
            })
        );
        
        console.log('- Bot is Admin (Ultimate Check):', isBotAdmin);
        
        // Debug match details
        adminList.forEach(admin => {
            const adminNumber = admin.id.match(/\d+/)?.[0];
            const matches = botNumbers.filter(botNum => {
                const botNumberOnly = botNum.match(/\d+/)?.[0];
                return admin.id === botNum || (adminNumber && botNumberOnly && adminNumber === botNumberOnly);
            });
            if (matches.length > 0) {
                console.log(`✅ MATCH FOUND: Admin ${admin.id} matches Bot ${matches[0]}`);
            }
        });
        ...
        console.log('❌ Ultimate Bot Admin Check Error:', error);
        return false;
    }
};
// ========== BACKUP: FORCE ADMIN CHECK (NUCLEAR OPTION) ==========
const ForceAdminCheck = async () => {
    if (!isGroup) return false;
    
    try {
        // Coba delete pesan dummy untuk test permission
        const testMessage = await Zion.sendMessage(m.chat, { text: "Link detect akan segera di hapus...." });
        ...
        try {
            await Zion.sendMessage(m.chat, {
                delete: {
                    ...
                    participant: testMessage.key.participant || Zion.user.id
                }
            });
            console.log('✅ Force Admin Check: Bot CAN delete messages');
            return true;
        } catch (deleteError) {
            console.log('❌ Force Admin Check: Bot CANNOT delete messages');
            return false;
        }
        
    } catch (error) {
        console.log('❌ Force Admin Check Error:', error);
        return false;
    }
};
...
const qkontak = {
    ...
    // tambahkan message object jika diperlukan
...
newsletterJid: "120363403261361342@newsletter",
...
contextInfo: {
    externalAdReply: {
        ...
        body: `🕑 ${waktu}`,
        sourceUrl: '',
        thumbnailUrl: "https://files.catbox.moe/fqdiqa.jpg",
...
global.menfess = global.menfess ? global.menfess : {}
    ...
    try {
        ...
        const logic = "kamu adalah Xenovia ai, asisten ai yang di buat oleh kennzy, kamu bisa mempromosikan link group https://chat.whatsapp.com/FKLOiDRsK4T9DcSvFGxNBx tetapi agak jarang saja jangan terlalu sering mempromosikan nya, kamu adalah asisten ai cerdas yang cantik dan ramah kamu suka emot 🐱😄😆, jangan sering menyapa orang, dan kamu bisa bahasa Jepang tapi jangan kebanyakan jepang";
...
case 'help': {
    ...
    try {
        ...
        const ultimateCheck = await UltimateBotAdminCheck();
        const forceCheck = await ForceAdminCheck();
        
        let debugInfo = `🚨 *EMERGENCY DEBUG*\n\n`;
        
        debugInfo += `🤖 *BOT IDENTITY:*\n`;
        debugInfo += `• botNumber: ${botNumber}\n`;
        debugInfo += `• Zion.user.id: ${Zion.user?.id}\n`;
        debugInfo += `• Number only: ${botNumber?.match(/\d+/)?.[0]}\n\n`;
        
        debugInfo += `⚡ *FINAL RESULTS:*\n`;
        debugInfo += `• Ultimate Check: ${ultimateCheck}\n`;
        debugInfo += `• Force Check: ${forceCheck}\n`;
        debugInfo += `• Recommendation: ${ultimateCheck || forceCheck ? 'SHOULD WORK' : 'PROBLEM DETECTED'}\n\n`;
        
        debugInfo += `👑 *ADMIN NUMBERS:*\n`;
        adminList.forEach((admin, i) => {
            const adminNum = admin.id.match(/\d+/)?.[0];
            const botNum = botNumber?.match(/\d+/)?.[0];
            const isMatch = adminNum === botNum ? ' ⭐ MATCH!' : '';
            debugInfo += `${i + 1}. ${admin.id}${isMatch}\n`;
        });
        
        if (!ultimateCheck && !forceCheck) {
            debugInfo += `\n🔧 *SOLUTION:*\n`;
            debugInfo += `1. Remove bot dari admin\n`;
            debugInfo += `2. Tunggu 10 detik\n`;
            debugInfo += `3. Add bot sebagai admin lagi\n`;
            debugInfo += `4. Test lagi dengan .emergency`;
        }
        
        reply(debugInfo);
        
    } catch (error) {
        reply(`💥 Emergency Debug Error: ${error.message}`);
    }
}
break;
case 'refreshadmin':
case 'reloadbot': {
    ...
    try {
        // Force refresh group metadata
        const newGroupMeta = await Zion.groupMetadata(m.chat);
        const newAdminList = newGroupMeta.participants.filter(v => v.admin !== null).map(v => v.id);
        const newgroupAdmins = groupAdmins.includes(botNumber);
        
        reply(`🔄 *ADMIN STATUS REFRESHED*\n\n• Bot Admin Status: ${newBotAdminStatus}\n• Total Admins: ${newAdminList.length}\n\nSilakan test antilink lagi!`);
    } catch (error) {
        reply('Error refresh: ' + error.message);
    }
}
break;
case "reactch":
case "rch": {
    ...
    if (!text) return m.reply("Contoh:\n.reactch https://whatsapp.com/channel/xxx/123 ❤️kenn\n.reactch https://whatsapp.com/channel/xxx/123 ❤️ken|5");
...
break
            default:
               ...
               // ========== FIXED BOT ADMIN CHECK & ANTILINK ==========



// ========== UPDATED ANTILINK WITH ULTIMATE CHECK - ALL TYPES ==========

// Anti Wa.me - Ultimate Fix
if (global.antiWame) {
    const wamePattern = /(https?:\/\/)?(www\.)?(wa\.me|whatsapp\.com\/qr|api\.whatsapp\.com)/gi;
    if (wamePattern.test(budy)) {
        const isBotReallyAdmin = await UltimateBotAdminCheck();
        
        if (!isBotReallyAdmin) {
            const forceResult = await ForceAdminCheck();
            if (!forceResult) {
                return reply('⚠️ Bot bukan admin! Pastikan bot dijadikan admin grup terlebih dahulu.');
            }
        }
        
        if (isGroupAdmins) return reply('Admin bebas kirim link wa.me');
        if (isOwner) return reply('Owner bebas kirim link apapun');
        ...
        try {
            ...
            await Zion.sendMessage(m.chat, {
                text: `⚠️ *WA.ME LINK DIHAPUS!*\n\n@${m.sender.split("@")[0]} mengirim link wa.me`,
                contextInfo: { mentionedJid: [m.sender] }
            }, {quoted: m});
            
        } catch (error) {
            console.log('❌ Error deleting wa.me:', error);
            reply('Link wa.me terdeteksi tapi gagal dihapus! Error: ' + error.message);
        }
        return;
    }
}

// Anti Link Group - Ultimate Fix
if (global.Antilinkgc) {
    const gcPattern = /(https?:\/\/)?(www\.)?(chat\.whatsapp\.com|whatsapp\.com\/invite)/gi;
    if (gcPattern.test(budy)) {
        const isBotReallyAdmin = await UltimateBotAdminCheck();
        
        if (!isBotReallyAdmin) {
            const forceResult = await ForceAdminCheck();
            if (!forceResult) {
                return reply('⚠️ Bot bukan admin! Tidak bisa hapus link grup.');
            }
        }
        
        if (isAdmins) return reply('Admin bebas kirim link grup');
        if (isOwner) return reply('Owner bebas kirim link apapun');
        ...
        try {
            ...
            await Zion.sendMessage(m.chat, {
                text: `⚠️ *LINK GRUP DIHAPUS!*\n\n@${m.sender.split("@")[0]} mengirim link grup WhatsApp`,
                contextInfo: { mentionedJid: [m.sender] }
            }, {quoted: m});
            
        } catch (error) {
            console.log('❌ Error deleting group link:', error);
            reply('Link grup terdeteksi tapi gagal dihapus! Error: ' + error.message);
        }
        return;
    }
}

// Anti Link Channel - Ultimate Fix
if (global.Antilinkch) {
    const chPattern = /(https?:\/\/)?(www\.)?whatsapp\.com\/channel/gi;
    if (chPattern.test(budy)) {
        const isBotReallyAdmin = await UltimateBotAdminCheck();
        
        if (!isBotReallyAdmin) {
            const forceResult = await ForceAdminCheck();
            if (!forceResult) {
                return reply('⚠️ Bot bukan admin! Tidak bisa hapus link channel.');
            }
        }
        
        if (isAdmins) return reply('Admin bebas kirim link channel');
        if (isOwner) return reply('Owner bebas kirim link apapun');
        ...
        try {
            ...
            await Zion.sendMessage(m.chat, {
                text: `⚠️ *LINK CHANNEL DIHAPUS!*\n\n@${m.sender.split("@")[0]} mengirim link channel`,
                contextInfo: { mentionedJid: [m.sender] }
            }, {quoted: m});
            
        } catch (error) {
            console.log('❌ Error deleting channel link:', error);
            reply('Link channel terdeteksi tapi gagal dihapus!');
        }
        return;
    }
}

// Anti Link Instagram - Ultimate Fix  
if (global.Antilinkig) {
    const igPattern = /(https?:\/\/)?(www\.)?(instagram\.com|instagr\.am)/gi;
    if (igPattern.test(budy)) {
        const isBotReallyAdmin = await UltimateBotAdminCheck();
        
        if (!isBotReallyAdmin) {
            const forceResult = await ForceAdminCheck();
            if (!forceResult) {
                return reply('⚠️ Bot bukan admin! Tidak bisa hapus link Instagram.');
            }
        }
        
        if (isAdmins) return reply('Admin bebas kirim link IG');
        if (isOwner) return reply('Owner bebas kirim link apapun');
        ...
        try {
            ...
            await Zion.sendMessage(m.chat, {
                text: `⚠️ *LINK INSTAGRAM DIHAPUS!*\n\n@${m.sender.split("@")[0]} mengirim link Instagram`,
                contextInfo: { mentionedJid: [m.sender] }
            }, {quoted: m});
            
        } catch (error) {
            console.log('❌ Error deleting IG link:', error);
            reply('Link Instagram terdeteksi tapi gagal dihapus!');
        }
        return;
    }
}

// Anti All Links - Ultimate Fix
if (global.Antilinkall) {
    const allLinkPattern = /(https?:\/\/[^\s]+)/gi;
    if (allLinkPattern.test(budy)) {
        const isBotReallyAdmin = await UltimateBotAdminCheck();
        
        if (!isBotReallyAdmin) {
            const forceResult = await ForceAdminCheck();
            if (!forceResult) {
                return reply('⚠️ Bot bukan admin! Tidak bisa hapus link.');
            }
        }
        
        if (isAdmins) return reply('Admin bebas kirim link apapun');
        if (isOwner) return reply('Owner bebas kirim link apapun');
        ...
        try {
            ...
            reply('Link terdeteksi tapi gagal dihapus!');
        }
        return;
    }
}
...
// ========== FORCE ADMIN REFRESH COMMAND ==========


// ========== DEBUGGING COMMAND ==========
// Tambahin case ini di switch statement untuk debug:
