...
const PAIRING_TIMEOUT_MS = Number(process.env.PAIRING_TIMEOUT_MS || 90_000);
...
function describeBaileysError(err) {
    if (!err) return { name: '', message: '', stack: '', statusCode: null, data: null, payload: null };
    const boom = err?.output?.statusCode ? err : (err?.cause?.output?.statusCode ? err.cause : null);
    return {
        name: err?.name || '',
        message: err?.message || String(err),
        stack: err?.stack || '',
        statusCode: boom?.output?.statusCode ?? err?.output?.statusCode ?? err?.statusCode ?? null,
        data: err?.data ?? boom?.output?.payload?.data ?? null,
        payload: boom?.output?.payload ?? err?.output?.payload ?? null,
    };
}

function logPairingDiagnostic(stage, err, extra = {}) {
    const d = describeBaileysError(err);
    log.error(`PAIRING DIAGNOSTIC [${stage}]`);
    if (d.statusCode !== null) log.error(`  statusCode : ${d.statusCode}`);
    if (d.name) log.error(`  name       : ${d.name}`);
    if (d.message) log.error(`  message    : ${d.message}`);
    if (d.data !== null && d.data !== undefined) log.error(`  data       : ${safeJson(d.data)}`);
    if (d.payload) log.error(`  payload    : ${safeJson(d.payload)}`);
    if (Object.keys(extra).length) log.error(`  context    : ${safeJson(extra)}`);
    if (d.stack) log.error(`  stack      : ${d.stack.split('\n').slice(0, 4).join(' | ')}`);
}
...
async function startBot() {
    ...
    sock.ev.on('connection.update', ({ connection, lastDisconnect }) => {
        ...
        if (connection === 'open') {
            ...
            log.success(`Fitur   : Admin · Tools · Media · Bot · Proteksi`);
        ...
        if (connection === 'close') {
            ...
            if (!state.creds.registered) {
                ...
                logPairingDiagnostic('CONNECTION_CLOSE', lastDisconnect?.error, {
                    statusCode: code,
                    reasonName,
                    message: errMsg,
                ...
                log.error(`❌ Koneksi putus saat pairing: ${reasonName} (kode ${code})${errMsg ? ` — ${errMsg}` : ''}`);
                if (code === 500) {
                    log.warn('🔎 Kode 500 saja belum cukup untuk menentukan penyebab. Diagnostic lengkap di atas adalah data yang perlu diperiksa.');
                    log.warn('🔎 ECONNRESET/ETIMEDOUT/DNS/TLS biasanya menunjuk jaringan server; payload/rejection WhatsApp menunjuk penolakan dari sisi WA.');
                }
                return;
            }
    ...
    // Simpan credential setiap kali Baileys mengubah auth state, sekaligus
    // tandai saat pairing benar-benar diterima oleh WhatsApp. Ini penting:
    // keluarnya pairing code SAJA belum berarti akun sudah ter-link.
    sock.ev.on('creds.update', async (creds) => {
        try {
            await saveCreds();
            if (creds?.registered === true || state.creds.registered === true) {
                log.success('🔐 Pairing diterima WhatsApp — credential akun sudah terdaftar dan disimpan.');
            }
        } catch (err) {
            log.error(`Gagal menyimpan credential: ${err.message}`);
        }
    });

    // ── Pairing Code nyata ─────────────────────────────────────────
    // Diagnostic-friendly pairing:
    // - 90s timeout by default (PAIRING_TIMEOUT_MS can override)
    // - NEVER delete session automatically because pairing timed out
    ...
    if (!state.creds.registered) {
        ...
        // IMPORTANT: Baileys tidak mengekspos WebSocket sebagai `sock.ws` pada
        // API publik. Versi v2 sebelumnya melakukan polling `sock.ws.readyState`,
        // sehingga nilainya selalu `unknown/null` dan requestPairingCode() tidak
        // pernah dipanggil. Untuk pairing code, tunggu event connection.update
        // (khususnya `qr`, yang menandakan socket sudah melewati tahap awal auth),
        // lalu panggil requestPairingCode() SEKALI.
        const pairingDeadline = Date.now() + PAIRING_TIMEOUT_MS;
        let lastPairingError = null;
        let code = null;
        let pairingRequested = false;
        let pairingReadySeen = false;

        const waitForPairingReady = () => new Promise((resolve, reject) => {
            let timer;
            const cleanup = () => {
                clearTimeout(timer);
            ...
            const onUpdate = (update) => {
                const { connection, qr, lastDisconnect: updateDisconnect } = update || {};
                if (qr) {
                    pairingReadySeen = true;
                    log.info('🔎 Baileys mengirim event QR/companion-ready — socket siap menerima requestPairingCode().');
                    cleanup();
                    resolve('qr');
                    return;
                }
                ...
                if (connection === 'close') {
                    const err = updateDisconnect?.error || lastDisconnect?.error;
                    cleanup();
                    reject(err || new Error('Koneksi ditutup sebelum socket siap untuk pairing code.'));
                }
            };
            sock.ev.on('connection.update', onUpdate);
            timer = setTimeout(() => {
                cleanup();
                reject(new Error('Tidak menerima event socket-ready (qr/open) sebelum timeout.'));
            }, Math.max(1, pairingDeadline - Date.now()));
        });

        try {
            // Event `qr` adalah readiness signal yang dipakai oleh banyak contoh
            // Baileys untuk memulai pairing-code flow. Jangan mengakses sock.ws.
            const readySignal = await waitForPairingReady();
            log.info(`🔎 Socket pairing-ready signal: ${readySignal}`);
        } catch (err) {
            lastPairingError = err;
            logPairingDiagnostic('PAIRING_SOCKET_NOT_READY', err, {
                timeoutMs: PAIRING_TIMEOUT_MS,
                remainingMs: Math.max(0, pairingDeadline - Date.now()),
                pairingReadySeen,
                registered: state.creds.registered,
            });
        }

        if (!lastPairingError && !state.creds.registered && !pairingRequested) {
            pairingRequested = true;
            try {
                const cleanNumber = String(NOMOR_HP).replace(/[^0-9]/g, '');
                log.info(`📡 Mengirim requestPairingCode() untuk ${cleanNumber}...`);
                code = await sock.requestPairingCode(cleanNumber);

                if (code) {
                    log.success('✅ WhatsApp mengeluarkan pairing code. Masukkan kode tersebut di HP sekarang.');
                } else {
                    lastPairingError = new Error('requestPairingCode() mengembalikan hasil kosong.');
                    logPairingDiagnostic('EMPTY_PAIRING_CODE', lastPairingError, {
                        pairingReadySeen,
                        registered: state.creds.registered,
                    });
                }
                ...
                lastPairingError = err;
                logPairingDiagnostic('REQUEST_PAIRING_CODE_FAILED', err, {
                    pairingReadySeen,
                    registered: state.creds.registered,
                    remainingMs: Math.max(0, pairingDeadline - Date.now()),
                });
        ...
        if (!code) {
            if (!lastPairingError) {
                lastPairingError = new Error('Pairing code tidak tersedia dan tidak ada error spesifik dari Baileys.');
            }
            logPairingDiagnostic('PAIRING_CODE_FAILED', lastPairingError, {
                timeoutMs: PAIRING_TIMEOUT_MS,
                pairingRequested,
                pairingReadySeen,
                registered: state.creds.registered,
            });
            log.error('❌ Pairing code gagal dibuat.');
            log.warn('⚠️ Session TIDAK dihapus otomatis dan bot TIDAK restart loop.');
            log.warn('➡️ Jika diagnostic berisi 428/400/500/515 atau stream:error, itu adalah error dari jalur koneksi/WhatsApp, bukan timeout buatan polling readyState.');
            try { sock.end?.(lastPairingError); } catch {}
            return;
        }

        const formatted = code?.match(/.{1,4}/g)?.join('-') ?? code;
        console.log('\n' + '╔' + '═'.repeat(52) + '╗');
        console.log(`║  🔑 FORSCIENO BOT — PAIRING CODE`.padEnd(53) + '║');
        console.log(`║  ${String(formatted).padEnd(48)}║`);
        console.log('╚' + '═'.repeat(52) + '╝');
        console.log('\n📲 CARA PAIRING WHATSAPP:');
        console.log('   1. Buka WhatsApp di HP nomor +' + NOMOR_HP);
        console.log('   2. Buka Setelan → Perangkat Tertaut');
        console.log('   3. Pilih Tautkan Perangkat → Tautkan dengan Nomor Telepon');
        console.log('   4. Masukkan kode di atas SEGERA.');
        console.log('   5. Tunggu log: 🔐 Pairing diterima WhatsApp.');
        console.log('   6. Lalu tunggu: WhatsApp TERHUBUNG.\n');
        // Do not request another code: the same socket now waits for device linking.
    }
...
async function gracefulShutdown(signalName) {
    if (shuttingDown) return; // cegah shutdown ganda jika sinyal masuk 2x
    shuttingDown = true;
