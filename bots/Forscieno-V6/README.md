# 🥶 My WhatsApp Bot — v3.2.1 Infinity Edition
...
FFmpeg sekarang dibawa melalui dependency `ffmpeg-static@5.3.0` sebagai fallback untuk Linux/Pterodactyl. Saat runtime bot mencari FFmpeg dengan urutan: `FFMPEG_BIN` → FFmpeg sistem/PATH → `ffmpeg-static`. Pada Termux/Android, FFmpeg sistem selalu diprioritaskan dan binary `ffmpeg-static` sengaja tidak digunakan karena binary tersebut ditujukan untuk Linux glibc, sedangkan Termux memakai Android/Bionic. Jadi di Termux tetap jalankan `pkg install ffmpeg`.

Untuk Pterodactyl, `npm install` akan mengunduh binary FFmpeg melalui dependency tersebut; tidak perlu memasang paket FFmpeg di dalam container selama arsitektur OS didukung. `ffmpeg-static` menyediakan binary Linux x64/arm64 dan versi FFmpeg yang dibundel adalah 6.1.1.
...
### ⚔️ RPG (sudah dihapus dari runtime bot)
...
### 🎮 Fun & Game (sudah dihapus dari runtime bot)
...
- Gambar brat digenerate lewat API remote (`brat.siputzx.my.id`), lalu dikonversi jadi sticker WebP di sisi bot memakai `FFmpeg sistem` — tidak butuh compiler/native build tambahan, instalasi tetap ringan untuk Pterodactyl.
...
**Penting:** bot menggunakan **WhatsApp Pairing Code nyata** melalui `sock.requestPairingCode()`. Nomor harus format internasional tanpa `+`, spasi, atau strip. Startup Variable yang digunakan adalah `NOMOR_HP` (lebih diutamakan daripada `setting.js`).
3. Jalankan bot — kode pairing akan muncul di console
4. Buka WA → Setelan → Perangkat Tertaut → Tautkan dengan Nomor Telepon
5. Masukkan kode pairing segera setelah muncul
6. Tunggu sampai console menampilkan `WhatsApp TERHUBUNG`.
...
> 💡 `.brat`/`.bratgreen`/`.bratwhite`/`.antinsfw` (untuk video) butuh ffmpeg. Secara default lewat `FFmpeg sistem` (sudah ada di `package.json`, otomatis terinstall lewat `npm install` di atas — tidak perlu compiler/toolchain tambahan). Kalau package ini gagal terinstall/jalan di server, command terkait akan memberi pesan jelas saat dipanggil (bukan crash bot) — dan sejak v3.2.1, bot juga otomatis fallback ke ffmpeg sistem (dari PATH) kalau ada.
...
> 💡 **Kenapa perlu `pkg install ffmpeg` manual di Termux?** `FFmpeg sistem` di `package.json` men-download binary ffmpeg yang dikompilasi untuk Linux glibc biasa — binary itu **tidak bisa jalan** di Android/Termux (Termux pakai Bionic libc, beda dari glibc), meskipun `npm install`-nya sendiri tidak akan error. Sejak v3.2.1, bot otomatis fallback ke ffmpeg SISTEM (hasil `pkg install ffmpeg` di atas) kalau binary `FFmpeg sistem` terdeteksi tidak valid — jadi tidak perlu setting tambahan apa pun, command `.brat`/`.antinsfw` (bagian video) tetap berfungsi normal di Termux.
...
- ✅ **Dukungan Termux (Android)** — lihat subbagian "📱 Termux (Android)"
  ...
  binary bawaan `FFmpeg sistem` tidak jalan (kasus umum di Android, karena
