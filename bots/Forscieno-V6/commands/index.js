...
import { sendMainMenu, sendAdminMenu, sendToolsMenu, sendMediaMenu, sendBotMenu, sc } from './menu.js';
...
reg(['owner', 'dev', 'developer'], async (ctx) => {
...
// di dalamnya (adminCommands, toolsCommands, mediaCommands, dst). Ini akurat
...
const CATEGORY_ORDER = ['🛡️ Admin', '🛠️ Tools', '🖼️ Media', '🎵 Musik & Download', '👑 Jabatan', '🖥️ Panel', '🤖 Bot', '📦 Lainnya'];
...
//
// Daripada nulis ulang manual deskripsi utuk command yang terdaftar (rawan typo &
// bakal nyimpang dari sendAdminMenu/sendToolsMenu/dst begitu salah satu diedit
// di masa depan), fungsi ini "membaca" teks yang SUDAH ditulis manusia di
// sendAdminMenu/sendToolsMenu/sendMediaMenu/sendBotMenu/
// sendAdminMenu (commands/menu.js) — satu sumber tunggal dipakai bareng
// oleh menu kategori yang aktif DAN .allmenu, jadi otomatis selalu sinkron.
//
...
// Legacy guild aliases were removed with the game subsystem.
...
async function getMenuKnowledgeBase() {
    ...
    for (const sendFn of [sendAdminMenu, sendToolsMenu, sendMediaMenu, sendBotMenu]) {
        let raw = '';
...
『 領域展開 — Semua Jurus Terungkap 』`;
    ...
    // (Tools, Media, dst — bisa 200+ command), list dipecah lagi per HURUF
    ...
    // menuKB membaca sendAdminMenu/sendToolsMenu/sendMediaMenu/sendBotMenu/
...
`╭─〔 *${sc(cat)}* 〕─╮ _(${list.length})_\n┣➤ ${prefix}cpanel — buka menu lengkap (create/kelola server, role, dst)\n┗━━━━━━━━━━━━━━━━⊱`
        ...
        if (cat === '🛡️ Admin') {
            ...
            // Kategori LAIN (Tools, Media, Musik & Download, Bot,
...
`╭─〔 *${sc(cat)}* 〕─╮ _(${list.length})_\n${body}\n┗━━━━━━━━━━━━━━━━⊱`
    ...
    // dipakai juga oleh .menu) — kirim dengan thumbnail bot & fallback
