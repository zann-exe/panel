...
const FFMPEG_MISSING_HINT = '\n\n_(Fitur ini butuh FFmpeg sistem — pastikan `ffmpeg` tersedia di PATH)_';
...
async function withMediaEffect(ctx, cmd, allowedTypes, typeLabel, transformFn, resultKind, extraCaption = '') {
    ...
    try {
        ...
        await ctx.reply(`❌ Gagal proses media: ${err.message}${err.message.includes('FFmpeg') ? '' : FFMPEG_MISSING_HINT}`);
    }
}
